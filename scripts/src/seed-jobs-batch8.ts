...
// ── Italy (new batch — different companies & cities) ──
const italyJobs = [
  // Magazzino
...
async function main() {
  console.log("🚀 Seeding batch8 jobs...\n");
  let count = 0;

  const insertBatch = async (jobs: any[], country: string, lang: "it" | "en") => {
    for (const j of jobs) {
      const desc = lang === "it"
  ...
  await insertBatch(germanyJobs2, "DE", "en");
  await insertBatch(franceJobs2, "FR", "en");
  await insertBatch(spainJobs2, "ES", "en");
  await insertBatch(ukJobs2, "GB", "en");
  await insertBatch(chJobs2, "CH", "en");
  await insertBatch(nlJobs2, "NL", "en");
  await insertBatch(beJobs2, "BE", "en");
  await insertBatch(atJobs2, "AT", "en");
  await insertBatch(ieJobs2, "IE", "en");
  await insertBatch(ptJobs2, "PT", "en");
  await insertBatch(plJobs2, "PL", "en");
