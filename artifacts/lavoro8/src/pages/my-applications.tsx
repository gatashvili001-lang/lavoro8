...
export default function MyApplicationsPage() {
  ...
  const { data: applications, isLoading } = useQuery<MyApplication[]>({
    ...
    queryFn: () => customFetch("/api/my-applications"),
    enabled: !!isSignedIn,
    retry: false,
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full px-4 py-8">
        ...
        {(!applications || applications.length === 0) ? (
          <Card className="p-12 text-center">
            ...
            <p className="font-medium text-muted-foreground">Nessuna candidatura ancora</p>
            <Button variant="outline" className="mt-4" onClick={() => navigate("/jobs")}>Cerca lavoro</Button>
          </Card>
        ) : (
          <div className="space-y-3">
            {applications.map(app => {
              ...
              const Icon = cfg.icon;
              const hasReply = app.status === "replied" || app.status === "accepted" || app.status === "rejected";
              return (
                <Card
                  key={app.id}
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          <p className="font-semibold text-sm leading-tight">{app.jobTitle ?? "Posizione"}</p>
